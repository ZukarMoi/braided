...
- **Vault 連携**: `/api/vault/sync` ← ローカル dev サーバー (`local_server.py`) 経由で Obsidian に書き出し（本番では不使用）
...
ローカル dev サーバー (`local_server.py` + `vault.py`) が Obsidian の vault フォルダに書き出す。
